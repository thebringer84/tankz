...
import {terrainHeight,MAP_SIZE,seededRandom} from './config.js';
...
export function buildTerrain(scene,world,textures,materials,props,entities){
 ...
 const geo=new THREE.PlaneGeometry(MAP_SIZE,MAP_SIZE,180,180);geo.rotateX(-Math.PI/2);const p=geo.attributes.position;
 ...
 textures.height.repeat.set(12.1,12.1);
 ...
 const ruins=[[-17,-13],[15,-29],[38,29],[-37,30],[1,52],[-58,-49],[59,-57]];
 ...
 for(const [x,z,sx,sy,sz] of [[-5,8,1.5,.38,1.6],[6,13,1.8,.42,1.8],[8,4,2,.5,1.9],[-8,-1,2.2,.55,2.2],[11,20,2.4,.5,2.1]]){const mesh=new THREE.Mesh(rockGeo,materials.rock);mesh.scale.set(sx,sy,sz);mesh.rotation.y=.4;addProp(mesh,x,terrainHeight(x,z)+sy*.55,z,sx,sy,sz,Infinity,false,30,true,'rock');}

 for(let i=0;i<64;i++){const x=(rand()-.5)*195,z=(rand()-.5)*195;if(Math.hypot(x,z-17)<12)continue;const sx=1+rand()*2.7,sy=.5+rand()*1.6,sz=.8+rand()*2.3;const mesh=new THREE.Mesh(rockGeo,materials.rock);mesh.scale.set(sx,sy,sz);mesh.rotation.y=rand()*6;addProp(mesh,x,terrainHeight(x,z)+sy*.65,z,sx*.78,sy*.75,sz*.78,Infinity,false,30,true,'rock');}
 ...
 const shrubMat=new THREE.MeshStandardMaterial({color:0x7d8058,roughness:1,flatShading:true});const shrubGeo=new THREE.IcosahedronGeometry(1,1),shrubs=new THREE.InstancedMesh(shrubGeo,shrubMat,1800);const d=new THREE.Object3D();let n=0;
 for(let i=0;i<300;i++){const x=(rand()-.5)*205,z=(rand()-.5)*205;if(Math.hypot(x,z-17)<7)continue;const size=.35+rand()*.75;for(let j=0;j<5;j++){d.position.set(x+(rand()-.5)*size,terrainHeight(x,z)+size*.33+rand()*.35,z+(rand()-.5)*size);d.scale.set(size*.42,size*(.2+rand()*.3),size*.32);d.rotation.set(rand(),rand()*6,rand());seatOnGround(d,shrubGeo,surfaceHeight,.07);shrubs.setMatrixAt(n++,d.matrix);}}
 const branches=new THREE.InstancedMesh(new THREE.CylinderGeometry(.025,.05,1,4),materials.canvas,500);let bn=0;const twigRand=seededRandom(919);for(let i=0;i<220;i++){const x=(twigRand()-.5)*205,z=(twigRand()-.5)*205;d.position.set(x,terrainHeight(x,z)+.35,z);d.scale.set(1,.6+twigRand()*.5,1);d.rotation.set((twigRand()-.5)*.5,twigRand()*6,(twigRand()-.5)*.5);seatOnGround(d,branches.geometry,surfaceHeight,.06);branches.setMatrixAt(bn++,d.matrix);}branches.count=bn;group.add(branches);shrubs.count=n;shrubs.castShadow=true;shrubs.receiveShadow=true;group.add(shrubs);
 // Distant ridge silhouettes frame the playable desert.
 for(let i=0;i<64;i++){const a=i/64*Math.PI*2,r=118+rand()*14;const m=new THREE.Mesh(rockGeo,materials.rock);m.position.set(Math.sin(a)*r,3,Math.cos(a)*r);m.scale.set(7+rand()*10,5+rand()*15,7+rand()*10);m.rotation.y=a;group.add(m);}
 ruts.props=props;ruts.ruins=ruins;return {group,terrain,ruins,surfaceHeight,shrubs,branches,ruts};
}
