...
export class Presentation {
 constructor(game){
  ...
  this.bloom=new UnrealBloomPass(new THREE.Vector2(1,1),.38,.55,1.15);this.composer.addPass(this.bloom);this.smaa=new SMAAPass();this.composer.addPass(this.smaa);this.output=new OutputPass();this.composer.addPass(this.output);
  // Keep grain in display space, after OutputPass tone mapping and conversion.
  this.grain=this.output;Object.assign(this.output.uniforms,{time:{value:0},strength:{value:.025}});
  this.output.material.fragmentShader=this.output.material.fragmentShader.replace('void main()', 'uniform float time,strength;\nvoid main()').replace(/}\s*$/, `float noise=fract(sin(dot(gl_FragCoord.xy+floor(time*24.),vec2(12.9898,78.233)))*43758.5453)-.5;float vignette=smoothstep(.2,.8,distance(vUv,vec2(.5)));gl_FragColor.rgb=gl_FragColor.rgb*(1.-vignette*.12)+noise*strength;}`);

 }
 resize(quality=this.quality||'high'){this.quality=quality;const g=this.game;this.composer.setPixelRatio(g.renderer.getPixelRatio());this.composer.setSize(innerWidth,innerHeight);}
 render(dt){const g=this.game,u=this.fog.uniforms,showroom=!!g.showroom&&['menu','garage'].includes(g.mode);this.smaa.enabled=showroom||this.quality!=='low';u.showroom.value=showroom?1:0;this.grain.uniforms.strength.value=showroom?.012:.025;this.bloom.strength=showroom?.24:.38;if(g.mode==='playing')g.visibility.animate(dt);u.fogEnabled.value=['playing','paused','results'].includes(g.mode)?1:0;u.visibility.value=g.visibility.texture;u.inverseProjection.value.copy(g.camera.projectionMatrixInverse);g.camera.updateMatrixWorld();u.cameraWorld.value.copy(g.camera.matrixWorld);this.grain.uniforms.time.value=g.time;g.infantry?.crowd.prepare();this.composer.render(dt);}
}
