...
export function createMarauder(materials,enemy=false,detail='low'){
 ...
 const maps=materials.marauder||{},armor=marauderArmor(maps,enemy,high),hazard=hazardPaint(maps,high),{dark,track}=materials,steel=high?materials.steel.clone():materials.steel;
 if(high){steel.roughness=.45;steel.metalness=.85;steel.envMapIntensity=1.1;}
 ...
 const glass=new THREE.MeshStandardMaterial({color:0x121a1c,roughness:.15,metalness:.7,emissive:0x2a2410,emissiveIntensity:.4});
 ...
 const pennant=new THREE.MeshStandardMaterial({color:enemy?0x2b2b28:0xa3322a,roughness:1,side:THREE.DoubleSide});
 ...
 for(const side of [-1,1]){
  for(const end of [-1,1]){
   ...
   const wheel=(z,y,r)=>{const group=new THREE.Group();group.position.set(side*1.3,y,z);body.add(group);wheels.push(group);
    if(high){for(const dx of [-.14,.14])cyl(group,track,dx,0,0,r,.2,'x');for(const s of [-1,1])cyl(group,armor,s*.245,0,0,r*.8,.014,'x');cyl(group,steel,side*.26,0,0,r*.34,.05,'x');cyl(group,dark,side*.29,0,0,r*.14,.02,'x');for(let k=0;k<6;k++){const a=k/6*Math.PI*2;bolt(group,side*.29,Math.sin(a)*r*.25,Math.cos(a)*r*.25);}}
    else{cyl(group,track,0,0,0,r,.48,'x');cyl(group,armor,side*.245,0,0,r*.8,.014,'x');cyl(group,steel,side*.26,0,0,r*.34,.05,'x');}
    batch(group);};
  ...
  cyl(body,soot,x,top+R,z-R-.06,.105,.12,'z');cyl(body,dark,x,top+R,z-R-.121,.08,.004,'z');
 ...
 for(const side of [-1,1]){
  ...
  for(let k=0;k<3;k++){const tube=cyl(turret,tubes,side*1.05,.82,.1+k*.15,.055,.28,'x');tube.rotation.z=side*(Math.PI/2-.5);}
 ...
 box(turret,metal,.62,1.0,-.85,.12,.24,.2);box(turret,armor,.62,1.36,-.85,.62,.48,.72);
 if(high){for(let i=0;i<4;i++)for(let j=0;j<3;j++){cyl(turret,dark,.43+i*.127,1.23+j*.13,-.485,.05,.012,'z',.05,12);ring(turret,steel,.43+i*.127,1.23+j*.13,-.482,.054,.008,'z',12);}rod(turret,steel,[.36,.9,-.6],[.4,1.14,-.6],.02);}
 else box(turret,dark,.62,1.36,-.487,.54,.4,.01);
 // Bustle rack with jerry cans and bedrolls, aerials and a pennant.
 ...
 const flag=add(turret,new THREE.ShapeGeometry(new THREE.Shape([new THREE.Vector2(0,0),new THREE.Vector2(0,-.22),new THREE.Vector2(-.42,-.11)])),pennant,-.86,2.48,-1.36);flag.rotation.y=-Math.PI/2;
 ...
 cyl(gun,armor,0,0,2.1,.15,2.9,'z',.13);cyl(gun,armor,0,0,1.75,.21,.5,'z');cyl(gun,armor,0,0,1.44,.15,.12,'z',.21);cyl(gun,armor,0,0,2.06,.21,.12,'z',.14);
 box(gun,metal,0,0,3.78,.56,.4,.56);cyl(gun,dark,0,0,4.065,.12,.01,'z');
 if(high){for(const z of [1.2,2.5,3.1])ring(gun,metal,0,0,z,z<2?.16:.145,.018);for(const s of [-1,1])for(const z of [3.64,3.9])box(gun,dark,s*.281,0,z,.012,.26,.14);box(gun,metal,0,.215,3.78,.5,.03,.5);for(const s of [-1,1])for(const y of [-.2,.2])bolt(gun,s*.4,y,.38,'z');}
 for(const s of [-1,1]){box(gun,armor,s*.72,-.04,.2,.34,.34,.6);cyl(gun,gunMetal,s*.72,-.04,1.0,.045,1.0,'z');cyl(gun,receiver,s*.72,-.04,1.52,.06,.1,'z');if(high)for(const z of [.62,.8,.98])ring(gun,gunMetal,s*.72,-.04,z,.05,.01,'z',12);}
