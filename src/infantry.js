 ...
 *coverSearch(s){const g=this.game,p=s.body.translation(),threat=s.ai.lastKnown.clone(),nav=this.navigation;nav.refresh();let best=null,score=Infinity;
 ...
 beginFrame(){this.frameBudget={routes:4,cover:32,ms:2};}
 endFrame(){this.frameBudget=null;}
 plan(){
  const budget=this.frameBudget||{routes:4,cover:32,ms:2};
  for(const [s,goal] of this.routeJobs){
   if(budget.routes<=0)break;
  ...
  while(budget.cover>0&&this.coverJobs.size&&performance.now()<deadline){
   ...
   if(s.dead||s.wounded||!s.ai.engaged||s.movementState?.tier===2)continue;
 ...
 update(frameDt){const g=this.game;this.movement.prepare();this.rebuildNeighbors();this.tick=(this.tick||0)+1;
  ...
  for(const [phase,s] of g.soldiers.entries()){if(s.dead)continue;
   const dt=frameDt;
   if(s.wounded){this.updateWounded(s,dt);continue;}const p=vec(s.body.translation()),a=s.ai;
   ...
   const interval=tier===0?3:tier===1?6:12;
   ...
   if(!s.motion||changed||(this.tick+phase)%interval===0||s.decisionElapsed>=.2){
    s.motion=this.decide(s,p);s.decisionElapsed=0;s.decisionState=a.state;s.decisionSight=!!a.sees;s.decisionDamage=damage;
   }
   s.decisionHazard=hazard;
   ...
   animateInfantry(s,dt,s.visibleToPlayer!==false||s.visibilityOpacity>.005||a.sees);
 ...
 wound(s,severity='leg',side='L'){restoreCrew(s.crew);
  if(s.dead||s.wounded)return;this.pose(s);const g=this.game;g.visibility?.release?.(s);s.wounded=true;s.crawlHeading=g.rand()*Math.PI*2;s.crawlPhase=g.rand()*Math.PI*2;s.crawlRate=3.4+g.rand()*2.2;s.crawlPace=.75+g.rand()*.5;s.crawlTurnIn=1.5+g.rand()*2.5;s.woundGrace=.7;s.woundType=severity;s.woundLife=severity==='lower'?4+g.rand()*2:18+g.rand()*8;s.bleedTimer=0;s.hp=Math.min(s.hp,severity==='lower'?5:14);s.gun.visible=false;s.flameFiring=false;s.aimTime=0;s.cover=null;
