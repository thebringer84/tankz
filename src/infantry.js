 ...
 *coverSearch(s){const g=this.game,p=s.body.translation(),threat=s.ai.lastKnown.clone(),nav=this.navigation;nav.refresh();let best=null,score=Infinity;
 ...
 wound(s,severity='leg',side='L'){
  if(s.dead||s.wounded||s.burning)return;this.crowd.restore(s);this.pose(s);restoreCrew(s.crew);const g=this.game;g.visibility?.release?.(s);g.audio.scream?.(s.root.position.distanceTo(g.player.root.position),s.index||0,'pain');g.audio.bodyImpact?.(s.root.position.distanceTo(g.player.root.position));s.wounded=true;s.crawlHeading=g.rand()*Math.PI*2;s.crawlPhase=g.rand()*Math.PI*2;s.crawlRate=3.4+g.rand()*2.2;s.crawlPace=.75+g.rand()*.5;s.crawlTurnIn=1.5+g.rand()*2.5;s.woundGrace=.7;s.woundType=severity;s.woundLife=severity==='lower'?4+g.rand()*2:18+g.rand()*8;s.bleedTimer=0;s.hp=Math.min(s.hp,severity==='lower'?5:14);s.gun.visible=false;s.flameFiring=false;s.aimTime=0;s.cover=null;
