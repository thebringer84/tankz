...
try{
 await page.goto('http://localhost:5173');await page.waitForFunction(()=>window.tankz?.game.running);await page.waitForTimeout(800);
 ...
 await page.locator('[data-action="deploy"]').click();assert.equal(await page.evaluate(()=>tankz.game.mode),'playing');assert.deepEqual(errors,[]);
