...
try{await page.goto(process.env.TANKZ_URL||'http://localhost:5173');await page.waitForFunction(()=>window.tankz?.game?.running);await page.evaluate(()=>tankz.game.selectTank('heavy'));await page.waitForTimeout(900);
 ...
 const gap=await page.evaluate(async()=>{const THREE=await import('/node_modules/.vite/deps/three.js');const g=tankz.game;g.root.updateMatrixWorld(true);return new THREE.Box3().setFromObject(g.player.root).min.y-new THREE.Box3().setFromObject(g.showroom.userData.turntable.getObjectByName('turntable-deck')).max.y;});assert.ok(Math.abs(gap)<.03,`turntable gap ${gap}`);
